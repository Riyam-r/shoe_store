...
class ShoeListFragment : Fragment() {
    ...
    private fun addNewShoeViewTest(shoe : Shoe) {
        Timber.i("addNewShoeView")
        Timber.i("first shoe " + shoe.name)

        //Craete horizantal view
        var container : LinearLayout = LinearLayout(context)
        container.orientation = LinearLayout.HORIZONTAL

        // Create Text views (key : value)
        var keyTextView = TextView(context)
        keyTextView.text = getString(R.string.shoe_name) + ":"
        keyTextView.textSize = 13f
        keyTextView.isAllCaps = true
        val params = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.WRAP_CONTENT,
                LinearLayout.LayoutParams.WRAP_CONTENT
        )
        ...
        params.setMargins(10,10,10,10)
        keyTextView.layoutParams = params


        var valueTextView = TextView(context)
        valueTextView.text = shoe.name
        valueTextView.textSize = 13f
        ...
        //get parent view & add container to it
        val linearLayout: LinearLayout = binding.llParent
